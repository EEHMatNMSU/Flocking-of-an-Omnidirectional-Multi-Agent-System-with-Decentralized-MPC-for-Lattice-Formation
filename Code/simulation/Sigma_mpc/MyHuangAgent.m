...
classdef MyHuangAgent < DoubleIntegratorAgent
    ...
    methods
        ...
        function obj = MyHuangAgent(id, param, initialPos, initialVel, cfg)
        ...
        function L = calculateL(obj, xi)
            ...
            for i = 1:size(qij_reshaped,1)
                %qij_reshaped(i,:)
                %sigmaNorm(qij_reshaped(i,:))
                %norm(qij_reshaped(i,:))
                qij_norm(i) = MysigmaNorm(qij_reshaped(i,:))/desiredDistances(i);
            end
