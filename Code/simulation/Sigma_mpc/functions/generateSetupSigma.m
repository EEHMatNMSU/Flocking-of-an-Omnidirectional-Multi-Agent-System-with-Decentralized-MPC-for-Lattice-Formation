...
function setup = generateSetupSigma(configFile, param, setVelocity, pos, vel, varargin)
switch nargin
    case 3
...
for i = 1:length(Agents)
    %Agents{i} = HuangAgent_Original(Network.getId(), param, pos(:,i), vel(:,i), configFile);
    Agents{i} = MyHuangAgent(Network.getId(), param, pos(:,i), vel(:,i), configFile);
end
setup.Agents = [Agents{:}];
